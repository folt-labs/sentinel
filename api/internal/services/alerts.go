...
func (s *AlertService) List(ctx context.Context, orgID uuid.UUID, status, severity string, limit, offset int) ([]models.Alert, error) {
	query := `SELECT id, organization_id, server_id, severity, status, title, description, triggered_at, acknowledged_at, resolved_at
...
func (s *AlertService) Get(ctx context.Context, orgID, alertID uuid.UUID) (*models.Alert, error) {
	...
	err := s.db.Pool.QueryRow(ctx,
		`SELECT id, organization_id, server_id, severity, status, title, description, triggered_at, acknowledged_at, resolved_at
...
// ProcessEvents checks events and creates alerts as needed
